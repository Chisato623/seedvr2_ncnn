...
int moments_to_latent(const ncnn::Mat& moments, ncnn::Mat& latent)
{
    ...
    for (int channel = 0; channel < 16; channel++)
    {
        ...
        {
            output[index] = mean[index] * SeedVR2VAE::scaling_factor();
        }
    }
    return 0;
}
...
int SeedVR2VAE::encode(const ncnn::Mat& video, ncnn::Mat& latent) const
{
    ncnn::Mat moments;
    ...
    return moments_to_latent(moments, latent);
}
